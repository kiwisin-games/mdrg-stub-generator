...
itemprefab0.Description = 'Permite modificar o tamanho das coxas até certo ponto.'
itemprefab0.Price = 200000
itemprefab0.SlotData = SlotEquipData.CreateInstance('ThighBodyModules')
itemprefab0.ItemModifiableVariables = {ItemModifiableVariableFactory.CreateInstanceFloatVariable('thighSize', SimpleLocalizedString.CreateInstanceReplacement('Tamanho da coxa'), 0.5)}
...
itemprefab1.Description = 'Permite modificar o tamanho das panturrilhas até certo ponto.'
itemprefab1.Price = 100000
itemprefab1.SlotData = SlotEquipData.CreateInstance('CalfBodyModules')
itemprefab1.ItemModifiableVariables = {ItemModifiableVariableFactory.CreateInstanceFloatVariable('calfSize', SimpleLocalizedString.CreateInstanceReplacement('Tamanho da panturrilha'), 0.5)}
...
itemprefab2.Description = 'Permite modificar o tamanho dos pés até certo ponto.'
itemprefab2.Price = 690420
itemprefab2.SlotData = SlotEquipData.CreateInstance('FeetBodyModules')
itemprefab2.ItemModifiableVariables = {ItemModifiableVariableFactory.CreateInstanceFloatVariable('feetLength', SimpleLocalizedString.CreateInstanceReplacement('Comprimento do pé'), 0.5), ItemModifiableVariableFactory.CreateInstanceFloatVariable('feetWidth', SimpleLocalizedString.CreateInstanceReplacement('Largura do pé'), 0.5)}
...
itemprefab3.Description = 'Permite que você modifique o tamanho de seus seios até certo ponto.'
itemprefab3.Price = 12000000
itemprefab3.SlotData = SlotEquipData.CreateInstance('BoobsBodyModules')
itemprefab3.ItemModifiableVariables = {ItemModifiableVariableFactory.CreateInstanceFloatVariable('boobSize', SimpleLocalizedString.CreateInstanceReplacement('Tamanho do peito'), 0.5)}
...
itemprefab4.Description = 'Permite que você modifique o tamanho de sua bunda até certo ponto.'
itemprefab4.Price = 4000000
itemprefab4.SlotData = SlotEquipData.CreateInstance('AssBodyModules')
itemprefab4.ItemModifiableVariables = {ItemModifiableVariableFactory.CreateInstanceFloatVariable('assSize', SimpleLocalizedString.CreateInstanceReplacement('Tamanho da bunda'), 0.5)}
...
itemprefab5.Description = 'Este item estranho diminui significativamente a luxúria do seu sexbot. Por causa do DRM interno, ele só pode ser usado uma vez.'
...
itemprefab6.Description = 'Este item estranho aumenta a luxúria do seu sexbot. Devido ao DRM interno, ele só pode ser usado uma vez. USE COM CUIDADO!!!'
...
browser = UiOverlay.OpenBrowser()
coroutine.yield(GameUtilities.WaitForSecondsYield(1))
browser.ChangeWebsite("bang.ic/email")



-- script_5.lua

browser.ChangeWebsite("casino.ic")
...
coroutine.yield(GameUtilities.WaitForSecondsYield(1))
browser = UiOverlay.OpenBrowser()
coroutine.yield(GameUtilities.WaitForSecondsYield(2))
...
gv.ForceTakeMoney(1000000)


-- script_14.lua

gv.ReportSleepEnd(false)


-- script_15.lua

return not not (gv.EverCameInBot)


-- script_16.lua

return not not (gv.EverStartedStream)


-- script_17.lua

return not not (gv.EverCameInBot and gv.EverStartedStream)


-- script_18.lua

return not not (gv.CheckFlagAfterRelative("dispenserempty24hours", 24*60))


-- script_19.lua

return not not (gv.RiotsActive)


-- script_20.lua

return not not (gv.RiotsAfter)


-- script_21.lua

return not not (gv.Hour <= 12)
...
return not not (gv.Hour < 5 or gv.Hour >=15)
...
return not not (gv.Stamina < 0.2)


-- script_26.lua

return not not (gv.WorkedTodayAlready)


-- script_27.lua

return not not (gv.DispenserLevel>0)


-- script_28.lua

return not not (gv.FishesCaught > 0)
...
return not not (gv.CheckFlag("JoinUs2"))


-- script_32.lua

return not not (gv.search > 0.5)


-- script_33.lua

return not not (gv.CheckFlag("hobo"))


-- script_34.lua

return not not (gv.CheckFlag("bigDaddysAdviceFinished"))


-- script_35.lua

return not not (gv.CheckFlag("bigDaddysAdvicePart2") and gv.Hour == 2)
...
return not not (gv.money > 1000000)
...
return not not (gv.IsThereCumOnHer)


-- script_42.lua

return not not (gv.CanStartStream)
